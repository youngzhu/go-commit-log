...
import (
	...
	api "github.com/youngzhu/go-commit-log/api/v1"
	"os"
	"path"
)

// wraps the index and store types to coordinate operations
// across the two
type segment struct {
	store                  *store
	index                  *index
...
// and returns the newly appended record's offset
...
func (s *segment) Read(off uint64) (*api.Record, error) {
	// translate the absolute offset into a relative
...
// Remove closes the segment and remove the index and store files
